...
public record InitializeValidator() {
    ...
    public Initialize validateInitialize(String input) {
        Initialize init = new Initialize(); // default type is JUST_BEFORE_VUSER_RUNS
        if (StringUtils.isBlank(input)) return init; // return default
        ...
        } else if (just.matches()) log.debug("Parsed JUST_BEFORE initialize - default type already set");
        else log.debug("Unrecognized pattern '{}', using default initialize type: {}", input, init.getType());
    ...
    private void setWaitAfterInit(Initialize init, Matcher matcher) {
        ...
        if (StringUtils.isNotBlank(waitGroup)) init.setWaitAfterInit(parseTimeInterval(waitGroup));
        else init.setWaitAfterInit(new TimeInterval());
    }
    ...
    public void validateInitializeActions(List<Action> actions) {
        ...
        if (initializes.isEmpty()) {
            log.warn("No Initialize action found. Adding default Initialize.");
            actions.add(1, Action.initializeDefault());
        } else if (initializes.size() > 1) {
            log.warn("Multiple Initialize actions found. Keeping first, removing others.");
